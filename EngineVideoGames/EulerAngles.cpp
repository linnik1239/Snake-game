...
void EulerAngles::alignedRot(int XorZ,float angle,int mode,glm::vec3 rotVec )
{
	float ang = angle;
	switch (XorZ)
	{
	...
	case yAxis1:
		if (mode == 0) {
			phi = glm::rotate(phi, ang, glm::vec3(0, 1, 0));
			//psi = glm::rotate(psi, -ang, glm::vec3(0, 1, 0));
		}
		else {
			phi = glm::rotate(glm::mat4(1), ang, glm::vec3(0, 1, 0)) * phi;
			//psi = glm::rotate(glm::mat4(1), -ang, glm::vec3(0, 1, 0)) * psi;
		}
		break;
	case zAxis12:
		if(mode == 0)
	...
	case yAxis2:
		if (mode == 0) {
			theta = glm::rotate(phi, ang, glm::vec3(0, 1, 0));
			//psi = glm::rotate(psi, -ang, glm::vec3(0, 1, 0));
		}
		else {
			theta = glm::rotate(glm::mat4(1), ang, glm::vec3(0, 1, 0)) * phi;
			//psi = glm::rotate(glm::mat4(1), -ang, glm::vec3(0, 1, 0)) * psi;
		}
		break;
	case zxAxis1:
