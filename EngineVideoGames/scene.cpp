...
Scene::Scene()
{
	...
	glLineWidth(3);
	cameras.push_back(new Camera(vec3(0, 0, 1.0f), vec3(0, 0, -1.0f), 60.0f, 0.1f, 100.0f, Viewport(), glm::vec3(0,0,20)));
	pickedShape = -1;
	depth = 0;
	cameraIndx = 0;
	xold = 0;
...
Scene::Scene(vec3 position, float angle, float near, float far, Viewport &vp,glm::vec3 lookAt)
{
	//verticesSize = sizeof(vertices)/sizeof(vertices[0]);
	//
	//indicesSize = sizeof(indices)/sizeof(indices[0]) ; 
	glLineWidth(6);
	cameras.push_back(new Camera(position, -position, angle, near, far, vp, lookAt));
	pickedShape = -1;
	depth = 0;
	cameraIndx = 0;
	xold = 0;
...
{
	cameras.push_back(new Camera(pos, -pos, fov, zNear, zFar, vp,glm::vec3(0,0,0)));
}
...
void Scene::Draw(int shaderIndx, int cameraIndx, int buffer, bool toClear, bool debugMode)
{
	...
	glm::mat4 matTranses[50];
	glm::mat4 Camera = cameras[cameraIndx]->makeTrans();
	...
	//Camera = Camera * shapes[chainParents[6]]->makeTrans();


	for (int i = 0;i < getNumCylinders() + 2; ++i) {

		mat4 Normal1 = mat4(1);
		pickedShape = i + 6;
		for (int j = i + 6; chainParents[j] > -1; j = chainParents[j])
		{
		...
		MVP1 = MVP1 * shapes[i + 6]->makeTransScale(mat4(1));
	...
	for (unsigned int i = 0; i<shapes.size(); i++)
	{
		if (shapes[i]->Is2Render())
		{
			...
			if (shaderIndx > 0)
			{
				Update(matTranses,MVP1, Normal1, shapes[i]->GetShader(),i, Camera);
				shapes[i]->Draw(shaders, textures, false);

			}
			else
			{ //picking
				Update(matTranses,MVP1, Normal1, 0,i, Camera);
...
void Scene::Draw2D(int shaderIndx, int cameraIndx, int buffer, bool toClear, bool debugMode)
{
	...
	if (isActive)
		Update2D(Normal, 0, shaderIndx); // Update2D(Normal, 01, shaderIndx);
	else
